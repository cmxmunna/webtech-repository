...
{
    $conn = db_conn();
    $selectQuery = "UPDATE internetpack SET name = ?, speed = ?, usertype = ?, conntype = ?, time = ?, support = ?, usertype = ?, included = ?, image = ?, price = ?, display = ? where pack_id = ?";
    try
    {
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
        $data['name'], $data['speed'], $data['usertype'], $data['conntype'], $data['time'], $data['support'], $data['usertype'], $data['included'], $data['image'], $data['price'], $data['display'], $pack_id]);
    }
    catch(PDOException $e)
    {
        echo $e->getMessage();
